...
Level.prototype.obstacleAt = function(pos, size) {
  ...
  for (var y = yStart; y < yEnd; y++) {
    for (var x = xStart; x < xEnd; x++) {
      ...
      if (fieldType && fieldType != "floater" && fieldType != "swim" && fieldType != "block")
      return fieldType;
    }
  }
};
...
var wobbleS = 4, wobbleD = 1;

School.prototype.act = function(step) {
  this.wobble += step * wobbleS;
  var wobbleSPos = Math.sin(this.wobble) * wobbleD;
  var wobbleSPosY = Math.cos(this.wobble) * wobbleD
  this.pos = this.basePos.plus(new Vector(wobbleSPos, wobbleSPosY));
};
...
Player.prototype.moveX = function(step, level, keys) {
  ...
  if(obstacle!="wall" && obstacle!="floater" && obstacle!="swim" && obstacle!="block")
    this.pos = newPos;
};
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if ((type == "water" || type == "fish") && (this.status == null || this.status == "color" || this.status == "colors")) {
    this.status = "lost";
    this.finishDelay = 1;
  }
  else if (type == "coin") {
